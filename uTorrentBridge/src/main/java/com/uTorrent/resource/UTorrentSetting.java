...
import com.uTorrent.resource.exception.InvalidSettingTypeException;
...
public class UTorrentSetting {
	...
	private final Logger logger = LoggerFactory.getLogger(UTorrentSetting.class);

	public UTorrentSetting(JSONArray array) {
		try {
			this._name = array.getString(0);
			this._type = Integer.parseInt(array.getString(1));
			this._value = array.getString(2);
		}
		catch (Exception e) {
			this.logger.error("Error while parsing settings: ",e);
		}
	}
	...
	public final boolean getBooleanValue() {
		return this._value.equals("true");
	}

	public void setValue(int value) throws InvalidSettingTypeException {
		if (this._type == INTEGER_TYPE) {
			this._value = Integer.toString(value);
		}
		else {
